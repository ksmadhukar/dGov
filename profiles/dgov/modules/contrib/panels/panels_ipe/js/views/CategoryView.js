...
(function ($, _, Backbone, Drupal, drupalSettings) {
  ...
  Drupal.panels_ipe.CategoryView = Backbone.View.extend(/** @lends Drupal.panels_ipe.CategoryView# */{
    ...
    */
    ...
    '  <input type="submit" value="' + Drupal.t('Search block ...') + '" />' +
    '</div>' +
    ...
    */
    ...
    '  <% if (name == "Custom") { %><div class="ipe-category-title"> <i class="fa-icons fa fa-th-large" aria-hidden="true"></i><span>Existing Block</span></div><% } %>' +
    '  <% if (name == "Lists (Views)") { %><div class="ipe-category-title"> <i class="fa-icons fa fa-list" aria-hidden="true"></i><span>List</span></div><% } %>' +
    '  <% if (name == "Forms") { %><div class="ipe-category-title"> <i class="fa-icons fa fa-file" aria-hidden="true"></i><span>Forms</span></div><% } %>' +
    '  <% if (name == "Menus") { %><div class="ipe-category-title"> <i class="fa-icons fa fa-bars" aria-hidden="true"></i><span>Menus</span></div><% } %>' +
    '  <% if (name == "Webform") { %><div class="ipe-category-title"> <i class="fa-icons fa fa-file-text" aria-hidden="true"></i><span>Webform</span></div><% } %>' +
    '  <% if (name != "Custom" && name != "Lists (Views)" && name != "Forms" && name != "Menus"&& name != "Webform") { %> <%- name %> <%} %>' +
    '</a>'
    ),
    ...
    */
    renderCategories: function () {
      ...
      if (this.activeCategory) {
        search_prompt = Drupal.t('Search block..');
      }
      else {
        search_prompt = Drupal.t('Search block..');
      }
    ...
    */
    displayForm: function (e) {
      ...
      if($('#popup').length>0){
        ...
        $('body').append('<div id="popup"></div><div class="form-overlay"></div>');
      }
    ...
    */
    loadForm: function (info, template) {
      ...
      self.$('.ipe-category-picker-top').empty();
      this.$('.ipe-category-picker-top').fadeOut('fast', function () {
        ...
        ajax.options.complete = function () {
          ...
          $('.ipe-category-picker-top.form-displayed').addClass('test');
          $('.ipe-category-picker-top.form-displayed').appendTo('#popup');
        };
