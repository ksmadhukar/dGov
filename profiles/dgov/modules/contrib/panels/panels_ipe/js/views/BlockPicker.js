...
(function ($, _, Backbone, Drupal, drupalSettings) {
  ...
  Drupal.panels_ipe.BlockPicker = Drupal.panels_ipe.CategoryView.extend(/** @lends Drupal.panels_ipe.BlockPicker# */{
    ...
    */
    ...
    '  <i class="fa-icons fa fa-plus-square" aria-hidden="true"></i>' +
    '  <%- name %>' +
    '</a>'
    ),
    ...
    */
    ...
    '<% if (typeof(plugin_id) !== "undefined") { %>' +
      '<h4>' + Drupal.t('<strong><%- label %></strong>') + '</h4>' +
     '<% } else { %>' +
      '<h4>' + Drupal.t('<strong><%- label %></strong>') + '</h4>' +
    '<% } %>' +
    '<div class="ipe-block-form ipe-form"><div class="ipe-icon ipe-icon-loading"></div></div>'
    ),
    template_backtoCreateBlock: _.template(
   '<div class="ipe-category-title" id="backtocreateblock" data-category="<%- name %>"><a href="#"><i class="fa fa-arrow-left" aria-hidden="true"></i></a><%- BackText %></div>'
    ),
      /**
    * @type {function}
    */
    template_blockSearch: _.template(
    '<div class="ipe-block-picker-search">' +
      '<span class="ipe-icon ipe-icon-search"></span>' +
      '<input type="text" placeholder="<%= search_prompt %>" />' +
    '</div>' +
    '<div class="ipe-block-picker-top"></div>' +
    '<div class="ipe-block-picker-bottom" tabindex="-1">' +
    '<div class="ipe-blocks"></div>' +
    '</div>'
    ),
    /**
    * @type {function}
    ...
    */
    events: {
      ...
      'keyup .ipe-block-picker-search input[type="text"]': 'searchBlocks'
    },
    ...
    */
    render: function () {
      ...
      var create_active = this.activeCategory === 'Add New Block';
      ...
      // region.
      this.$('.ipe-category-picker-top').prepend(this.template_backtoCreateBlock({BackText:this.activeCategory}));


      if (create_active) {
        // Hide the search box.
        this.$('.ipe-category-picker-search').hide();
        this.$('.ipe-category-picker-top').append(this.template_blockSearch({search_prompt: 'Search Block..'}));
    ...
    searchBlocks: function (e) {
      var enteredText = e.currentTarget.value.toLowerCase();
      this.$(".ipe-category-picker-top > *:not('.ipe-block-picker-search')").remove();
      this.contentCollection.each(function (block_content_type) {
        var template_vars = block_content_type.toJSON();
        // Reduce the length of the description if needed.
        template_vars.trimmed_description = template_vars.description;
        ...
        var templateVal = template_vars.label.toLowerCase();
        if(templateVal.indexOf(enteredText)>-1){
         this.$('.ipe-category-picker-top').append(this.template_content_type(template_vars));
        }
      }, this);
        this.$('.ipe-category-picker-top').prepend(this.template_backtoCreateBlock({BackText:this.activeCategory}));
