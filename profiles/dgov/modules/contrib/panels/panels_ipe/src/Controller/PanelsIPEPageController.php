<?php
  ...
  class PanelsIPEPageController extends ControllerBase {
    ...
    public function getBlockPlugins($panels_storage_type, $panels_storage_id) {
      ...
      $alias = str_replace($base_url,"",$request->getUri());
      $path = \Drupal::service('path.alias_manager')->getPathByAlias($alias);
      $panelizer_default =  str_replace("/admin/panels_ipe/variant/panelizer_default/*node:","",$path);
      $nid =  strstr($panelizer_default, ":",true);
      if(!is_numeric($nid)){
        $path_to_block =  str_replace("/admin/panels_ipe/variant/panelizer_field/node:","",$path);
        $nid =  strstr($path_to_block, ":",true);
      }
      ...
      foreach ($definitions as $plugin_id => $definition) {
       ...
       if($definition['category'] == "Custom" || $definition['category'] == "Menus" || $definition['category'] == "Forms" || $definition['category'] == "Webform" || $definition['category'] == "Lists (Views)" || $definition['category'] == "System"){
         ...
         if (isset($node_new) && $node_new->bundle() == "bdx_market") {
           if ($block) {
             if (!strpos($block->bundle(), "community") && !strpos($block->bundle(), "home")) {
               $data[] = [
                 'plugin_id' => $plugin_id,
         ...
         elseif (isset($node_new) && $node_new->bundle() == "bdx_community") {
           if ($block) {
             if (!strpos($block->bundle(), "market") && !strpos($block->bundle(), "home")) {
               $data[] = [
                 'plugin_id' => $plugin_id,
                 'label' => $definition['admin_label'],
                 'category' => $definition['category'],
         ...
         elseif (isset($node_new) && $node_new->bundle() == "bdx_home") {
           if ($block) {
             if (!strpos($block->bundle(), "market") && !strpos($block->bundle(), "community")) {
               $data[] = [
                 'plugin_id' => $plugin_id,
                 'label' => $definition['admin_label'],
                 'category' => $definition['category'],
         ...
         if ($block) {
           if (!strpos($block->bundle(), "market") && !strpos($block->bundle(), "community") && !strpos($block->bundle(), "home")) {
             $data[] = [
               'plugin_id' => $plugin_id,
               'label' => $definition['admin_label'],
               'category' => $definition['category'],
    ...
    public function getBlockContentTypes($panels_storage_type, $panels_storage_id) {
      ...
      $path = \Drupal::service('path.alias_manager')->getPathByAlias($alias);
      $panelizer_default =  str_replace("/admin/panels_ipe/variant/panelizer_default/*node:","",$path);
      $nid =  strstr($panelizer_default, ":full:default/block_content/types",true);
      if(!is_numeric($nid)){
        $path_to_block =  str_replace("/admin/panels_ipe/variant/panelizer_field/node:","",$path);
        $nid =  strstr($path_to_block, ":full/block_content/types",true);
      }
      ...
      foreach ($types as $id => $definition) {
        if(isset($node) && $node->bundle() == "bdx_market") {
          if(!strpos($id,"community") && !strpos($id,"home")){
            $data[] = [
              'id' => $definition->id(),
        ...
        }elseif(isset($node) && $node->bundle() == "bdx_community") {
          if(!strpos($id,"market") && !strpos($id,"home")){
            $data[] = [
              'id' => $definition->id(),
              'revision' => $definition->shouldCreateNewRevision(),
              'label' => $definition->label(),
        ...
        }elseif(isset($node) && $node->bundle() == "bdx_home") {
          if(!strpos($id,"market") && !strpos($id,"community")){
            $data[] = [
              'id' => $definition->id(),
              'revision' => $definition->shouldCreateNewRevision(),
              'label' => $definition->label(),
          ...
          if(!strpos($id,"market") && !strpos($id,"community") && !strpos($id,"home")) {
            $data[] = [
              'id'          => $definition->id(),
              'revision'    => $definition->shouldCreateNewRevision(),
              'label'       => $definition->label(),
